...
// Regression coverage for #3260. In the file list, a very long filename
// used to expand its row cell and push the kind / mtime / menu columns
// off-screen. The CSS fix locks the name cell to `min-width: 0` so the
// grid row truncates the name with the existing `text-overflow: ellipsis`
// on `.df-row-name` instead of growing the cell. The JSX fix adds
// `title={f.name}` so the browser surfaces the full filename on hover
// even when the visible text is truncated. (`<DfPreview>` already renders
// the full name with `word-break: break-word` for users who open the
// preview pane.)
//
...
describe('DesignFilesPanel long filename truncation (#3260)', () => {
  ...
  it('keeps the truncate-friendly DOM structure (.df-file-row > .df-row-name-btn > .df-row-name-wrap > .df-row-name)', () => {
    const { container } = renderPanel([file({ name: LONG_NAME })]);
    // The CSS fix relies on this nesting: the grid row's name cell is
    // min-width:0, the inner wrap is min-width:0 / max-width:100%, and
    ...
    expect(row).toBeTruthy();
    const btn = row!.querySelector('button.df-row-name-btn');
